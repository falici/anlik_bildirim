...
import { Plus, Pencil, Trash2, Building2, Check, X, ToggleLeft, ToggleRight } from 'lucide-react'
...
export default function KurumlarPage() {
  ...
  const openNew = () => { setEditing(null); setForm({ ad: '', aciklama: '' }); setShowForm(true); setError('') }
  const openEdit = (k: Kurum) => { setEditing(k); setForm({ ad: k.ad, aciklama: k.aciklama || '' }); setShowForm(true); setError('') }
  ...
  const toggleAktif = async (k: Kurum) => {
    await fetch('/api/admin/kurumlar', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: k.id, ad: k.ad, aktif: !k.aktif })
    })
    load()
  }

  const remove = async (id: string) => {
    if (!confirm('Bu kurumu silmek istiyor musunuz? Tüm event ve QR verileri silinecek.')) return
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={form.ad} onChange={e => setForm({ ...form, ad: e.target.value })}
                  ...
                  placeholder="Örn: Grand Otel İstanbul" />
              </div>
              <div>
